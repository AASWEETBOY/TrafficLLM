...
def td_evaluation(predict_responses, target_responses, label_file, evaluation_result_file):
    ...
    metrics = {
        "accuracy": float(accuracy_score(labels, pred_fixed)),
        "precision_weighted": float(precision_score(labels, pred_fixed, average='weighted')),
        "recall_weighted": float(recall_score(labels, pred_fixed, average='weighted')),
        "f1_weighted": float(f1_score(labels, pred_fixed, average='weighted')),
    }
    confusion = confusion_matrix(labels, pred_fixed)
    cls_report = classification_report(labels, pred_fixed, digits=4)
    ...
    with open(evaluation_result_file, 'w', encoding="utf-8") as f:
        ...
        print("acc: %.10f" % metrics["accuracy"], file=f)
        print("precision: %.10f" % metrics["precision_weighted"], file=f)
        print("recall: %.10f" % metrics["recall_weighted"], file=f)
        print("f1: %.10f" % metrics["f1_weighted"], file=f)
        print("metrics_json: %s" % json.dumps(metrics, ensure_ascii=False), file=f)
